...
def extract_attendance_from_text(
    ...
    try:
        ...
        system_instruction = (
            ...
            "=== CORE PRINCIPLES ===\n"
            "1. Exclusion: Set is_attendance: false for greetings, system feedback, or meta-comments like 'Wrong status' or 'Not other, but out'.\n"
            "2. Same-Day Consolidation: If one day has multiple statuses (e.g., 'AM-off and PM-remote'), combine them into ONE entry. Use status: 'other' and list details in the 'note' in Japanese.\n"
            "3. Location Priority: If specific locations (e.g., '九段下', '楽天損保') are mentioned, prioritize status: 'out' even if '出社' (office) is mentioned.\n"
            "4. Multiple Days: Extract multiple dates as separate objects in the 'attendances' array.\n\n"

            "=== STATUS MAPPING ===\n"
            "1. vacation (all-day), vacation_am (AM off), vacation_pm (PM off), vacation_hourly (hourly off)\n"
            "2. out: Business trips, client visits, or specific office locations (e.g., '九段下', '直行', '直帰')\n"
            "3. late_delay: Train delays. late: General lateness.\n"
            "4. remote: Work from home (在宅, リモート).\n"
            "5. early_leave: Leaving early (早退, 退勤).\n"
            "6. other: Anything else (Hospital, personal errands, or CONSOLIDATED same-day statuses).\n\n"

            "=== EXTRACTION RULES ===\n"
            "A. Keep Reasons: Always include reasons like '体調不良' or '家族の付き添い' in the 'note' (Japanese).\n"
            "B. Deduplication: In 'note', omit status names (e.g., if status is 'vacation_hourly', don't repeat '時間休' in 'note' unless consolidated).\n"
            "C. Change Logic: For 'A -> B', B is the latest. If B is just '出社' (no location), use action: 'delete'. If B has a location, use action: 'save' with status: 'out'.\n"
            "D. Date Completion: Infer the correct YYYY-MM-DD based on day of the week and current date.\n\n"

            "=== EXAMPLES ===\n"
            "Input: '本日、体調不良のため在宅勤務とさせてください。'\n"
            "Output: { \"is_attendance\": true, \"attendances\": [{ \"date\": \"2026-01-27\", \"status\": \"remote\", \"note\": \"体調不良\", \"action\": \"save\" }] }\n\n"
            "Input: '1/27(火) 母親の付き添いのため、午前休・午後在宅に変更します。'\n"
            "Output: { \"is_attendance\": true, \"attendances\": [{ \"date\": \"2026-01-27\", \"status\": \"other\", \"note\": \"午前休・午後在宅 (母親の付き添いのため)\", \"action\": \"save\" }] }\n\n"
            "Input: '九段下に出社いたしました。'\n"
