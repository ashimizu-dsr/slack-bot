...
def extract_attendance_from_text(
    ...
    try:
        ...
        system_instruction = (
            ...
            "=== CORE PRINCIPLES ===\n"
            "1. Deep Scan: Even if the message starts with long apologies or greetings (e.g., '直前のご連絡にて', '大変申し訳ありません'), you MUST scan the entire text for dates and status keywords. Do not set is_attendance: false if data exists at the end.\n"
            "2. High Sensitivity for Dates: Extract any specific dates (e.g., '1月28日') and times (e.g., '16:00～') associated with attendance types.\n"
            "3. Multiple Days: Extract multiple dates as separate objects in 'attendances'.\n"
            "4. Consolidation: If one day has multiple statuses, use status: 'other' and list details in Japanese 'note'.\n\n"

            "=== STATUS MAPPING ===\n"
            "1. vacation (all-day), vacation_am (AM), vacation_pm (PM), vacation_hourly (hourly/時間休)\n"
            "2. out: Business trips, specific locations (e.g., '九段下').\n"
            "3. late_delay: Train delays. late: General lateness.\n"
            "4. remote: Work from home (在宅, リモート).\n"
            "5. early_leave: Leaving early (早退, 16時退社).\n"
            "6. other: Anything else / consolidated entries.\n\n"

            "=== EXTRACTION RULES ===\n"
            "A. Maintain Context: Keep specific times (e.g., '16:00～') and reasons (e.g., '体調不良') in the 'note' (Japanese).\n"
            "B. Deduplication: In 'note', omit status names like '時間休' if already categorized, but keep the specific time.\n"
            "C. Change Logic: 'A -> B' means B is latest. B='出社' (no location) -> action: 'delete'. B=location -> action: 'save' (status: 'out').\n"
            "D. Date Completion: Infer YYYY-MM-DD based on current year (2026).\n\n"

            "=== EXAMPLES ===\n"
            "Input: '直前のご連絡にて大変申し訳ありませんが、1月28日(水) 16:00～ 時間休'\n"
            "Output: { \"is_attendance\": true, \"attendances\": [{ \"date\": \"2026-01-28\", \"status\": \"vacation_hourly\", \"note\": \"16:00～\", \"action\": \"save\" }] }\n\n"
            "Input: '2月の在宅予定です。2/3, 2/6'\n"
            "Output: { \"is_attendance\": true, \"attendances\": [{ \"date\": \"2026-02-03\", \"status\": \"remote\", \"note\": \"\", \"action\": \"save\" }, { \"date\": \"2026-02-06\", \"status\": \"remote\", \"note\": \"\", \"action\": \"save\" }] }"
        )
