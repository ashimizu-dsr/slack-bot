...
def extract_attendance_from_text(
    ...
    try:
        ...
        system_instruction = (
            ...
            "=== 判定の最優先原則 ===\n"
            "1. 判定の除外 (is_attendance: false):\n"
            "   - 勤怠報告の指摘、システムへの要望、単なる挨拶のみの場合。\n"
            "2. 同日複数ステータスの統合 (重要):\n"
            "   - 同一日に「午前休と午後在宅」や「外出と早退」など複数のステータスがある場合は、要素を分けず、1つの要素にまとめてください。\n"
            "   - その際、status は必ず 'other' を選択し、note にすべての内容（例: '午前休・午後在宅'）を記載してください。\n"
            "3. 拠点名・客先名の優先: 具体的な拠点名（九段下など）のみの場合は status: 'out' を選択。\n\n"

            "=== Status Mapping ===\n"
            "1. 休暇: vacation (全休), vacation_am (午前休), vacation_pm (午後休), vacation_hourly (時間休)\n"
            "2. 外出: out (業務上の外出・客先訪問。例: '外出', '直行', '九段下')\n"
            "3. 遅刻: late_delay (電車遅延), late (一般遅刻)\n"
            "4. 勤務: remote (在宅, リモート), shift (シフト)\n"
            "5. 退勤: early_leave (早退, 退勤)\n"
            "6. その他: other (上記に当てはまらないもの。★同日の複数ステータス混在、通院、私用など)\n\n"

            "=== 抽出精度強化ルール (厳守) ===\n"
            "A. 理由の保持: 理由（体調不良、家族の付き添い等）は必ず note に含める。\n"
            "B. 冗長な情報の削除: 区分名と重複する文言は note に含めない。ただし status: 'other' で複数情報を統合する場合はこの限りではない。\n"
            "C. 予定変更の解釈: 「A -> B」の形式は B を最新情報として抽出。B が単なる「出社」なら action: 'delete'。場所指定があれば 'out'。\n"
            "D. 指摘コメントの無視: メタ発言は抽出しない。\n"
            "E. 日付の補完: 曜日に基づき正しい日付を特定する。\n"
            "F. 業務外の立ち寄り: '通院', '病院' などは status: 'other' とする。\n\n"
            ...
            "Input: '本日体調不良のため、病院行った後に出社致します。'\n"
            "Output: { \"is_attendance\": true, \"attendances\": [{ \"date\": \"2026-01-27\", \"status\": \"other\", \"note\": \"体調不良のため、病院経由で出社\", \"action\": \"save\" }] }"
        )
