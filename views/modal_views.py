...
# 4. レポート & セットアップ
# ==========================================
def build_daily_report_blocks(header: str, section_data: Dict[str, List[Dict]]) -> List[Dict[str, Any]]:
    blocks = [
    ...
    for section_name, records in section_data.items():
        ...
        if not records:
            ...
            lines = []
            for r in records:
                status_jp = STATUS_TRANSLATION.get(r['status'], r['status'])
                note = f" ({r['note']})" if r.get('note') else ""
                lines.append(f"• <@{r['user_id']}> - {status_jp}{note}")
            text += "\n".join(lines)
